...
sys_dump_allocated(void)
{
  ...
  if (argint(1, &len) < 0)
    return -1;
  return dump_allocated(frames, len);
}
...
int sys_sem_init(void)
{
  int *sem_id;
  int count;
  if(argint(0, (int*)&sem_id) < 0 || argint(1, &count) < 0)
    return -1;
